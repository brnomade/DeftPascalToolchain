...
def getSourceFileNameFromArguments( theArguments ):
	# Return a source file name derived from the script arguments
	if theArguments.type.strip() == "CODE":
		return theArguments.file.strip().split(".")[0] + ".pas"
	#end if
	if theArguments.type.strip() == "MODULE":
		return theArguments.file.strip().split(".")[0] + ".mod"
	#end if
#end function
...
def getDskFileNameFromArguments( arguments ):
    ...
    return arguments.dsk.strip().split(".")[0] + ".dsk"
...
def getDependencyList( aFileName, theArguments, aConfiguration ):
	...
	try:
		...
		sourcefile = os.path.join(aConfiguration["PROJECT SETTINGS"]["PROJECTS_HOME"], theArguments.folder, aFileName)
		try:
			lines = open(sourcefile).read().splitlines()
		except FileNotFoundError:
			print("")
			print("Aborting execution. Library file not found.")
			print('"' + aFileName + '" not located in "' + aConfiguration["REPOSITORY SETTINGS"]["REPOSITORY_FOLDER"] + '" or "' + aConfiguration["PROJECT SETTINGS"]["PROJECTS_HOME"] + '".')
			sys.exit()
		#end try
	#end try
	...
	while lines[index].startswith("%") :
		if ("%C" in lines[index]):
			...
			importList = importList + getDependencyList(library + "." + extension, theArguments, aConfiguration)
		#end if
		index = index + 1
	#end while
	return importList
...
def getIndirectDependencyList( aList, theArguments, aConfiguration ):
	# Identify all libraries used by the libraries used by the source code file
	importList = list()
	for libFile in aList :
		for tempFile in getDependencyList(libFile, theArguments, aConfiguration).strip().split():
...
def processSourceCodeDependencies( aConfiguration, theArguments ):
	...
	indirectImportList = getIndirectDependencyList( directImportList, theArguments, aConfiguration )
...
def startAndWaitCompilationViaAutoIT( aConfiguration, theArguments ):
	...
	oscommand = oscommand + " " + theArguments.file.strip().split(".")[0] + "/" + theArguments.file.strip().split(".")[1]
	if theArguments.type.strip() == "CODE" :
		oscommand = oscommand + " " + theArguments.file.strip().split(".")[0] + "/OBJ"
	#end if
	if theArguments.type.strip() == "MODULE" :
		oscommand = oscommand + " " + theArguments.file.strip().split(".")[0] + "/LIB"
	#end if
	oscommand = oscommand + " " + theArguments.file.strip().split(".")[0] + "/LST"
	print(oscommand)
	if subprocess.run(oscommand, shell=True).returncode == 1 :
		print("")
...
def refreshDskWithAllRelevantFiles( aDependencyList, aConfiguration, theArguments ):
	...
	aSourceFileName = getSourceFileNameFromArguments(theArguments) 
	aPrjFileName = getPrjFileNameFromArguments(theArguments) 
	aDskFileName = getDskFileNameFromArguments(theArguments) 
	#
	deleteFileInDsk( aSourceFileName, aDskFileName, theArguments.folder, aConfiguration )
	copyFileToDsk( aSourceFileName, theArguments.folder, aDskFileName, theArguments.folder, aConfiguration )
	#
...
def initialise_script_arguments_parser():
    ...
    parser = argparse.ArgumentParser(description="Compile the target Deft Pascal source and all modules from which it depends.")
    ...
    parser.add_argument("dsk",help="DSK file where the source file will be placed. File extension is not required and is assumed to be '.DSK'. Filepath is not needed as the location of the file is derived from the configuration (ini) file.")
    parser.add_argument("-type", choices=['CODE','MODULE'],default='CODE',help="Type of source file being compiled. {CODE} File is a '.PAS' code and therefore compiled as a stand-alone program. {MODULE} File is a '.MOD' code and therefore will be compiled as a library.")
...
def presentScriptSettings( aConfiguration, theArguments ):
	...
	try:
		...
		print("SOURCE TYPE:", theArguments.type)
...
# SCRIPT BEGIN
presentScriptTitle()
inputArguments = initialise_script_arguments_parser().parse_args()
