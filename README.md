...
- Due to the emulation based nature of the solution, the AUTOIT scripts depend heavily on screen coordinates and graphics card configuration to find out what and when to type on Mame. I have not tested the scripts in any other machine yet. So there is a chance that the autoit scripts will need refactoring once tests are done. 
